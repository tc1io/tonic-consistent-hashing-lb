...
//use tonic::transport::Executor;
use crate::executor::{Executor,SharedExec};
...
impl Future for ResponseFuture {
    type Output = Result<Response<hyper::Body>, Error>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let val = ready!(Pin::new(&mut self.inner).poll(cx)).map_err(Error::from_source)?;
        Ok(val).into()
    }
}
